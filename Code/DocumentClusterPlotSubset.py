...
alpha = alpha.sample(n = 300, random_state = 42)
...
def tsne_plot_2d(title, embeddings, words=[], a=1):
    with open('./DocCarClusteringSubset.csv', mode='w', encoding='UTF-8', newline='') as file:
        ewrite = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        ewrite.writerow(['Index', 'Title'])
        ...
        for i, word in enumerate(words):
            ...
            ewrite.writerow([i, words_ak[i]])
        plt.title(title)
        plt.grid(True)
        plt.savefig("DocCarClusteringSubset.png", format='png', dpi=150, bbox_inches='tight')
        plt.show()
