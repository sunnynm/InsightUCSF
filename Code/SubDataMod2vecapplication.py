...
import pickle
filesave = './manuallyrefineddatamodtopiclist.pickle'
with open(filesave, 'rb') as fp:
...
with open('./DataModKeywordDistSub.csv', mode='w', encoding='UTF-8', newline='') as file:
	ewrite = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
	ewrite.writerow(['Document', 'Phrase', 'EhrVal', 'RefinedWord', 'RefinedVal', "ManRefWord", "ManRefVal"])
		...
		for j in c:
			...
			for gram in range(1, 4):
				...
				for phrase in phrasetotry:
					try:
						cval = model.similarity('ehr', phrase)
						cword = 'ehr'
					except:
						cval = 0
