...
import re
import pandas as pd
subdf = pd.read_csv('./SubAbstracts.csv')
with open('./DataMLProbwordDistSub.csv', mode='w', encoding='UTF-8', newline='') as file:
	ewrite = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
	ewrite.writerow(['Document', 'Phrase', 'ClasVal', 'RefinedWord', 'RefinedVal', "ManRefWord", "ManRefVal"])
		...
		for j in c:
			...
			for gram in range(1, 4):
				...
				for phrase in phrasetotry:
					try:
						cval = model.similarity('classification', phrase)
						cword = 'classification'
					except:
						cval = 0
