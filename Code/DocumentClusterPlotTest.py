...
import pandas as pd
df = pd.read_csv("./MutliKeywordDist.csv") 
...
def tsne_plot_2d(title, embeddings, words=[], a=1):
    with open('./DocCarClusteringTestV2.csv', mode='w', encoding='UTF-8', newline='') as file:
        ...
        plt.title(title)
        plt.grid(True)
        plt.savefig("DocCarClusteringTestV2.png", format='png', dpi=150, bbox_inches='tight')
        plt.show()
